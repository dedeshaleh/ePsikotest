      ...
      <div class="row">
        ...
        <section class="col-lg-12 connectedSortable">
          ...
          <div class="card">
            ...
            <!-- /.card-header -->
            <div class="card-body">
              <button class="btn btn-sm btn-success" data-target="#tambah" data-toggle="modal" id="btn_tambah"><i class="fas fa-plus"></i> Tambah Level</button>
              ...
              <table class="table table-sm table-stripped" id="data_user">
                  ...
                  <tbody>
                    <?php $no = 1; foreach ($DataLevel as $val) { ?>
                      <tr>
                        <td><?=$no++?></td>
                        <td><?=$val->nama_level?></td>
                        <td><button type="button" onclick="GetEdit('<?=$val->id_level?>')" class="btn btn-xs btn-warning btnEdit" ><i class="fa fa-edit"></i></button> || <button type="button" onclick="GetDelete('<?=$val->id_level?>')" class="btn btn-xs btn-danger btnDelete"><i class="fa fa-trash"></i></button></td>
                      </tr>
                    <?php } ?>
                  </tbody>
                </table>
...
<!-- Modal -->
<form action="<?=base_url()?>Master/Level/Tambah" method="post" enctype="multipart/form-data">
...
<!-- Modal -->
<form action="<?=base_url()?>Master/Level/Edit" method="post" enctype="multipart/form-data">
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="row">
          ...
          <div class="form-group">
            <label for="">Nama Level</label>
            <input type="text" name="nama_level" id="nama_level_edit" class="form-control">
            <input type="hidden" name="id_level" id="id_level_edit" class="form-control">
          </div>
          </div>
          <div class="col-6">
...
<script>
  ...
  function GetEdit(id) {
    $.ajax({
      url : "<?=base_url()?>Master/Level/GetData",
      type : "POST",
      dataType : "JSON",
      data : {id:id},
      success : function (data) {
        $("#modalEdit").modal("show");
        document.getElementById("nama_level_edit").value = data.nama_level;
