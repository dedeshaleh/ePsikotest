      ...
      <div class="row">
        ...
        <section class="col-lg-12 connectedSortable">
          ...
          <div class="card">
            ...
            <!-- /.card-header -->
            <div class="card-body">
              <a href="<?=base_url()?>Master/Soal/IsiSoal/<?=$id_subsoaldetail?>" class="btn btn-sm btn-success"  id="btn_tambah" ><i class="fas fa-plus"></i> Tambah Soal</a>
              ||
...
<script>
  ...
  $(document).ready(function() {
    ...
    $('#DataSubSoal').DataTable( {
		destroy: true,
		ajax: {
            'url'	: "<?=base_url()?>Master/Soal/Data_SubDetail",
            'type': "POST",
        ...
        render: function (data, type, row, meta) {
          if (row.TypeJawaban == 1) {
			...
			{ 
				data: 'id_subdetail',
				render : function (data, type, row, meta) {
          if (menu_akses.edit_level == 'Y' && row.TypeJawaban != 3) {
            // var edit = '<button type="button" onclick="GetEdit('+row.id_subdetail+')" class="btn btn-xs btn-warning"><i class="fa fa-edit"></i></button>';
            var edit = '<a href="<?=base_url()?>Master/Soal/EditSoal/'+row.id_subdetail+'" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i> Edit</a>'
          }else{
            var edit = '<button type="button" onclick="GetView(`'+row.id_subdetail+'`)" class="btn btn-xs btn-warning"><i class="fas fa-eye"></i></button>';
          }
          ...
          if (menu_akses.delete_level == 'Y') {
            var dele = '<button type="button" onclick="GetDelete(`'+row.id_subdetail+'`)" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></button>';
          }else{
            var dele = ''
          }
  ...
  function GetDelete(id) {
    $.ajax({
      ...
      success : function (data) {
        ...
        document.getElementById("id_subdetail_del").value = data.id_subdetail;
  ...
  $(document).ready(function(){
        ...
        function GetView(id) {
            ...
            $.ajax({
            url : "<?=base_url()?>Master/Soal/GetDataSubDetailView",
            type : "POST",
            dataType : "JSON",
            data : {id:id},
            success : function (data) {
              ...
              for (let i = 0; i < data.length; i++) {
                     ...
                     `
                     <tr>
                      <td>`+data[i].IsiDetailSoal+`</td>
                      <td style='text-align: center;'>`+data[i].Plus+`</td>
                      <td style='text-align: center;'>`+data[i].Minus+`</td>
                    </tr>
                     `           
              }
