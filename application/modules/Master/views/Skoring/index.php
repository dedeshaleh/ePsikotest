  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <input type="hidden" name="IdSkoring" id="IdSkoring_edit">
...
<form action="<?=base_url()?>Master/Skoring/Tambah" method="post" enctype="multipart/form-data">
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="form-group">
          <label for="">Nama Skoring</label>
          <input type="text" name="NamaSkoring" id="NamaSkoring" class="form-control">
        </div>
...
<form action="<?=base_url()?>Master/Skoring/Delete" method="post" enctype="multipart/form-data">
<div class="modal fade" id="modalDel" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="modalDelLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <label for="">Apakah anda yakin ingin mendelete Menu ini : <span id="NamaSkoring_del"></span></label>
        <input type="hidden" name="IdSkoring" id="IdSkoring_del">
...
<script>
  ...
  $(document).ready(function() {
    ...
    $('#DataSkoring').DataTable( {
		destroy: true,
		ajax: {
            'url'	: "<?=base_url()?>Master/Skoring/Data_Detail",
            'type': "POST",
        ...
        render : function (data, type, row, meta) {
          if (row.StatusSkoring == 1) {
			...
			{ 
				data: 'IdSkoring',
				render : function (data, type, row, meta) {
            var data_soal = '<a href="<?=base_url()?>Master/Skoring/SubSkoring/'+row.IdSkoring+'" type="button" class="btn btn-xs btn-success"><i class="fa fa-book"></i> Tambah Skoring</a>';
          if (menu_akses.edit_level == 'Y') {
            var edit = '<button type="button" onclick="GetEdit(`'+row.IdSkoring+'`)" class="btn btn-xs btn-warning"><i class="fa fa-edit"></i></button>';
          }else{
            var edit = '';
          }
          ...
          if (menu_akses.delete_level == 'Y') {
            var dele = '<button type="button" onclick="GetDelete(`'+row.IdSkoring+'`)" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></button>';
          }else{
            var dele = ''
          }
  ...
  function GetDelete(id) {
    $.ajax({
      ...
      success : function (data) {
        $("#modalDel").modal("show");
        document.getElementById("NamaSkoring_del").innerHTML = data.NamaSkoring;
        document.getElementById("IdSkoring_del").value = data.IdSkoring;
