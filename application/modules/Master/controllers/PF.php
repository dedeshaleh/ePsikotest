...
class PF extends MY_Controller
{
    //
    public $CI;
    ...
    public function index()
    {
        ...
        $data['judul'] = "Master PF16";
        $data['DataPF'] = $this->M_PF->GetPF();
        
        $data['akses'] = $this;
        $this->template->backend('PF/index',$data);
    }
    ...
    public function GetData()
    {
        ...
        $q1 = $this->db->query("SELECT * FROM mst_pf16_header a LEFT JOIN mst_pf16_detail b ON a.IdPFHeader = b.IdPFHeader where a.IdPFHeader = '$id'")->result();
    ...
    public function Tambah()
    {
        ...
        $IdPFHeader = uniqid(true);
        $data = array(
            'IdPFHeader' => $IdPFHeader,
        ...
        $this->db->insert("mst_pf16_header", $data);


        foreach ($_POST['KodePF'] as $key => $value) {
            $DataDetail[] = array(
                'IdPFHeader' => $IdPFHeader,
                'KodePF' => $_POST["KodePF"][$key],
                'NilaiPF' => $_POST["NilaiPF"][$key]
            );
        };

        $this->db->insert_batch("mst_pf16_detail", $DataDetail);
    ...
    {
        $IdPFHeader = $this->input->post('IdPFHeader');

        $this->M_PF->DeleteHeader($IdPFHeader);
        $this->M_PF->DeleteDetail($IdPFHeader);
        redirect("Master/PF");
